...
public class IntervalTraining {
	...
	public IntervalTraining() throws InterruptedException {
		...
		while (!response.equals("quit")) {
			...
			int note2 = note1 + allowedIntervals[(int)(Math.random() * allowedIntervals.length)];
	        ...
	        for (int i = 0; i <= StdAudio.SAMPLE_RATE * seconds; i++) {
	            StdAudio.play(0.5 * Math.sin(2*Math.PI * hz1 * i / StdAudio.SAMPLE_RATE));
	        }
	        ...
	        for (int i = 0; i <= StdAudio.SAMPLE_RATE * seconds; i++) {
	            StdAudio.play(0.5 * Math.sin(2*Math.PI * hz2 * i / StdAudio.SAMPLE_RATE));
	        }
	        ...
	        if(noteNameMatches(response, note2)) {
	        	...
	        	System.out.println("Nope. The interval was " + (note2 - note1) + ", and the note was " + getNoteName(note2) + ".");
	        }
	        System.out.println("-------------------------");
		}
	...
	private boolean noteNameMatches(String input, int halfStepsFromA) {
		...
		String[] noteArray =          {"A", "Bb", "B",  "C",  "C#", "D", "Eb", "E",  "F",  "F#", "G", "G#"};
		String[] secondaryNoteArray = {"A", "A#", "Cb", "B#", "Db", "D", "D#", "Fb", "E#", "Gb", "G", "Ab"};
		
		return input.equalsIgnoreCase(noteArray[halfStepsFromA]) || input.equalsIgnoreCase(secondaryNoteArray[halfStepsFromA]);
	}
